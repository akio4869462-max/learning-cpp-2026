...
int total_salary(const std::vector<Employee>& v) {
    ...
    int total = std::accumulate(v.begin(), v.end(), 0, [](int acc, const Employee& emp){ return acc + emp.salary;});
...
std::map<std::string, double> avg_by_dept(const std::vector<Employee>& v) {
    std::map<std::string, int> sum_map;
    std::map<std::string, int> count_map;
    std::map<std::string, double> result_map;

    for(const auto& emp : v){
        sum_map[emp.dept] += emp.salary;
        count_map[emp.dept] += 1;
    }
    for(const auto& emp : v){
        result_map[emp.dept] = (double)sum_map[emp.dept] / count_map[emp.dept];
    }
    return result_map;
}
...
std::optional<Employee> top_earner(const std::vector<Employee>& v) {
    auto it = std::max_element(v.begin(), v.end(), [](const Employee& a, const Employee& b){
