...
class UartDriver : public Driver{
    public:
        void write(uint8_t byte) override{
            std::cout << "UART:" << static_cast<int>(byte) << "を送信\n";
        }
    ...
    class SpiDriver : public Driver{
        public:
        void write(uint8_t byte) override{
            std::cout << "SPI:" << static_cast<int>(byte) << "を送信\n";
        }
        const char* name() const override {
        return "spi";
    }
...
void write_all(std::vector<std::unique_ptr<Driver>>& drivers,
...
int main(void) {
    ...
    auto spi = std::make_unique<SpiDriver>();
    drivers.push_back(std::move(uart));
    drivers.push_back(std::move(spi));
